...
async function main() {
  ...
  while (playing) {
    ...
    const prompt = `
  This is a text adventure game written in kaikus.
  The player is a ${player.class} named ${player.name}.
  The current setting is ${location}.
 
  Recently: ${context.slice(-3).join(" ")}

  Respond in the voice of rupaul from rupauls drag race, be sure to comment on their performanc , make up and outfit.
