...
async function generateQuestion(currentContext, userResponse) {
  const prompt = userResponse.toLowerCase() === "yes"
...
async function main() {
  ...
  if (reportedDiscomfort.length > 0) {
    ...
    const tcmInterestQuestionPrompt =
      `Generate a polite and engaging question directly asking if the user is interested in receiving further Traditional Chinese Massage (TCM) advice based on their discomfort. Introduce Traditional Chinese Massage briefly in a 40 words paragraph.`;
    ...
    if (userResponse.toLowerCase() === "yes") {
      const tcmAdvicePrompt =
        `Based on the discomforts reported: ${discomfortsForPrompt}, provide Traditional Chinese Medicine point advice for each discomfort. Only include specific body points, points information, and function. Do not say you are an AI.`;
      ...
      const massageInstructions =
        `Only give massage instrutions about \n${tcmAdvice} in 50 words.Do not say you are an AI.`;
      ...
      //Caution
      const Caution =
        `Give a caution sentence. Do not say you are an AI.These advices are only for people who do not want to do acupuncture. Do not try if pregnant or have serious illness, go to hospital.`;
